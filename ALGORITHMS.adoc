...
This document presents the algorithmic foundations of Eclexia’s runtime
system, including: (1) solution selection algorithms with complexity
analysis; (2) shadow price computation via linear programming; (3)
multi-objective optimization algorithms; (4) carbon-aware scheduling
algorithms; (5) type inference and constraint solving algorithms; (6)
resource profiling and prediction; and (7) amortized analysis for
adaptive execution. We provide rigorous complexity bounds and
correctness proofs for each algorithm.
...
. link:#2-solution-selection[Solution Selection]
. link:#3-shadow-price-computation[Shadow Price Computation]
. link:#4-linear-programming-algorithms[Linear Programming Algorithms]
. link:#5-multi-objective-optimization[Multi-Objective Optimization]
. link:#6-carbon-aware-scheduling[Carbon-Aware Scheduling]
. link:#7-type-inference-algorithms[Type Inference Algorithms]
. link:#8-dimension-inference-and-checking[Dimension Inference and
Checking]
. link:#9-constraint-solving[Constraint Solving]
. link:#10-resource-profiling[Resource Profiling]
. link:#11-adaptive-scheduling[Adaptive Scheduling]
. link:#12-amortized-analysis[Amortized Analysis]
. link:#13-approximation-algorithms[Approximation Algorithms]
. link:#14-online-algorithms[Online Algorithms]
. link:#15-complexity-summary[Complexity Summary]
...
*Input:* - Solutions `+S = {s₁, ..., sₙ}+` - Guards
`+G = {g₁, ..., gₙ}+` (boolean predicates) - Resource profiles
`+P = {p₁, ..., pₙ}+` where `+pᵢ : R → ℝ≥0+` - Current state
`+Σ : R → ℝ≥0+` - Budget `+B : R → ℝ≥0 ∪ {∞}+` - Objectives
`+O = {(minimize, r₁), (maximize, r₂), ...}+`

*Output:* - Index `+i*+` of optimal feasible solution
...
*Theorem 2.1 (Selection Correctness).* Algorithm 2.2 returns the index
minimizing weighted cost among feasible solutions.

_Proof:_ 1. Feasibility: Line 3 ensures only feasible solutions are
considered. 2. Optimality: Lines 10-16 find minimum over feasible set.
...
where: - `+x ∈ ℝⁿ+` is the selection vector (relaxed from binary) -
`+c ∈ ℝⁿ+` is the objective coefficient vector - `+A ∈ ℝ^{m×n}+` is the
resource consumption matrix: `+Aᵣᵢ = pᵢ(r)+` - `+b = B - Σ+` is the
...
*Complexity:* O(m³) using interior point methods, or O(2^m · poly(n,m))
using simplex.
...
*Theorem 3.1 (Shadow Price Sensitivity).* For small budget perturbation
`+Δb+`:
...
_Proof:_ By envelope theorem for linear programs. □

*Corollary 3.1.* Shadow price `+λᵣ+` represents the marginal value of
resource `+r+`:
...
*Complexity:* O(2^m) worst case, O(m² · n) average case (Spielman-Teng
smoothed analysis).
...
*Definition 5.1 (Pareto Dominance).* Solution `+s₁+` dominates `+s₂+`
(written `+s₁ ≻ s₂+`) if: - `+∀r ∈ O: fᵣ(s₁) ≤ fᵣ(s₂)+` (at least as
good on all objectives) - `+∃r ∈ O: fᵣ(s₁) < fᵣ(s₂)+` (strictly better
on at least one)
...
_Proof:_ If `+s*+` were dominated by `+s'+`, then
`+scalar_cost(s') < scalar_cost(s*)+` since weights are positive.
Contradiction. □
...
*Input:* - Tasks `+T = {t₁, ..., tₙ}+` with deadlines `+d_i+` and energy
requirements `+e_i+` - Carbon intensity forecast `+c(t) : [0, H] → ℝ⁺+`
over horizon H - Carbon budget `+C_max+`

*Output:* - Schedule `+σ : T → [0, H]+` (start times)

*Objective:* - Minimize total carbon: `+Σᵢ eᵢ · c(σ(tᵢ))+` - Subject to:
deadlines met, energy available
...
*Theorem 6.1 (Threshold Optimality).* For IID carbon intensity,
threshold policy is optimal.

_Proof:_ By optimal stopping theory. The problem is a stopping problem;
threshold policies are optimal for IID observations. □
...
*Theorem 7.1 (Principal Types).* If `+Γ ⊢ e : τ+` is derivable, then
Algorithm W computes a principal type `+τ_p+` such that `+τ = S(τ_p)+`
for some substitution `+S+`.

_Proof:_ By structural induction on `+e+`. Each step computes most
general unifier. □
...
*Definition 12.1 (Amortized Cost).* For sequence of operations,
amortized cost is:
...
*Theorem 12.1 (Amortized Bound).* Each adaptive selection has O(n·m)
amortized cost.

_Proof:_ 1. Actual cost: O(n·m) for selection 2. ΔΦ: At most -1 (one
unit of potential consumed) 3. Total: O(n·m) + O(1) = O(n·m) □
...
_Proof:_ Each operation pays O(n·m). Total potential decrease bounded by
initial potential. □
...
*Definition 14.1 (Competitive Ratio).* Online algorithm A is
c-competitive if:
...
*Theorem 14.1 (Carbon Competitive Ratio).* Algorithm 14.1 is
2-competitive.

_Proof Sketch:_ 1. For any task, either execute now or at best future
slot. 2. If we execute now at intensity c, OPT might wait for intensity
c’ ≤ c. 3. Worst case: c = 2c’ (we pay twice optimal). 4. Across all
tasks: Total ≤ 2 · OPT. □
