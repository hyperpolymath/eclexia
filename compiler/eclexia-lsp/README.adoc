...
Language Server Protocol (LSP) implementation for the Eclexia
...
* *Text Document Synchronization* - Keeps editor and server state in
sync
...
[arabic]
. Implement the LSP method in `+server.rs+`
. Update server capabilities in `+initialize()+`
. Add any necessary state to `+EclexiaLanguageServer+`
. Test with a compatible LSP client
...
Or use the
https://github.com/microsoft/language-server-protocol/tree/main/inspector[LSP
Inspector]for visual testing.
