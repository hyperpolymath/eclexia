...
This document is the authoritative specification for the Eclexia
...
[arabic]
. link:#lexical-structure[Lexical Structure]
. link:#syntax[Syntax]
. link:#type-system[Type System]
. link:#resource-semantics[Resource Semantics]
. link:#runtime-behavior[Runtime Behavior]
...
Eclexia uses bidirectional type checking. Judgment notation:
`+Γ ⊢ e : τ+`
...
Base dimensions: L (length), M (mass), T (time), I (current), Θ
(temperature), N (amount), J (luminosity)
...
Resources are declared with dimension, budget, and shadow price
function:
...
On function call with `+@requires(resource: amount)+`: 1. Check
`+usage + amount <= budget+` 2. If yes, `+usage += amount+` 3. Recompute
`+shadow_price+` based on new usage
...
*Instruction Set:* - `+LoadConst <value>+` - Push constant onto stack -
`+LoadLocal <index>+` - Push local variable - `+StoreLocal <index>+` -
Pop stack, store in local - `+Add+`, `+Sub+`, `+Mul+`, `+Div+` -
Arithmetic operations - `+Call <func_id>+` - Call function - `+Return+`
- Return from function - `+Jump <offset>+` - Unconditional jump -
...
*Value Stack:* For expression evaluation *Call Stack:* For function
calls and local variables *Heap:* For dynamically-allocated objects
(strings, arrays, etc.)

All values are 64-bit: - Integers: i64 - Floats: f64 - Pointers: 64-bit
addresses - Booleans: 0 (false) or 1 (true)
...
On `+ConsumeResource+` instruction: 1. Look up resource in table 2.
Check budget constraint 3. Update usage 4. Recompute shadow price
...
*Types:* - `+Option<T>+` - Optional value (Some or None) -
`+Result<T, E>+` - Fallible operation result (Ok or Err)

*Functions:* - `+print(s: String)+` - Output to stdout -
`+panic(msg: String)+` - Abort with error message -
`+assert(condition: Bool, msg: String)+` - Assert condition or panic

*Option Methods:* - `+is_some(self) -> Bool+` -
`+is_none(self) -> Bool+` - `+unwrap(self) -> T+` -
`+unwrap_or(self, default: T) -> T+`

*Result Methods:* - `+is_ok(self) -> Bool+` - `+is_err(self) -> Bool+` -
`+unwrap(self) -> T+` - `+unwrap_err(self) -> E+` -
`+unwrap_or(self, default: T) -> T+`

==== Collections Module (`+collections+`)

*Vec* - Dynamic array - `+new() -> Vec<T>+` -
`+push(&mut self, item: T)+` - `+pop(&mut self) -> Option<T>+` -
`+len(&self) -> Int+` - `+get(&self, index: Int) -> Option<T>+`

*HashMap<K, V>* - Hash table - `+new() -> HashMap<K, V>+` -
`+insert(&mut self, key: K, value: V)+` -
`+get(&self, key: K) -> Option<V>+` -
`+remove(&mut self, key: K) -> Option<V>+` -
`+contains_key(&self, key: K) -> Bool+`

*HashSet* - Hash set - `+new() -> HashSet<T>+` -
...
*Trigonometry:* - `+sin(x: Float) -> Float+` -
`+cos(x: Float) -> Float+` - `+tan(x: Float) -> Float+`

*Exponentials:* - `+exp(x: Float) -> Float+` -
`+log(x: Float) -> Float+` - `+pow(x: Float, y: Float) -> Float+`

*Rounding:* - `+floor(x: Float) -> Float+` - `+ceil(x: Float) -> Float+`
- `+round(x: Float) -> Float+`

*Constants:* - `+PI: Float = 3.14159265358979323846+` -
...
*JSON:* - `+read_json(path: String) -> Result<Value, String>+` -
`+write_json(path: String, value: Value) -> Result<(), String>+`

==== Text Module (`+text+`)

*String Operations:* - `+trim(s: String) -> String+` -
...
*Duration:* - `+from_secs(secs: Float) -> Duration+` -
`+from_millis(millis: Int) -> Duration+` - `+as_secs(self) -> Float+` -
`+as_millis(self) -> Int+`

*Instant:* - `+now() -> Instant+` - `+elapsed(self) -> Duration+` -
`+duration_since(self, earlier: Instant) -> Duration+`

*Functions:* - `+sleep(duration: Duration)+` -
...
Keywords reserved but not yet implemented: - `+async+`, `+await+` -
Asynchronous programming - `+unsafe+` - Unsafe operations - `+move+` -
Ownership transfer - `+ref+` - Reference types - `+mod+` - Module system
...
[arabic]
. *Type System:* Pierce, B. C. (2002). _Types and Programming
Languages_. MIT Press.
. *Shadow Pricing:* Dantzig, G. B., & Thapa, M. N. (1997). _Linear
Programming 1: Introduction_. Springer.
. *Dimensional Analysis:* Buckingham, E. (1914). "`On Physically Similar
Systems; Illustrations of the Use of Dimensional Equations.`" _Physical
Review_.
. *Resource-Aware Computing:* Hoffmann, J., & Hofmann, M. (2010).
"`Amortized Resource Analysis with Polymorphic Recursion and Partial
Big-Step Operational Semantics.`" _APLAS_.

'''''

*Document Version:* 1.0.0 *Eclexia Version:* 0.1.0 *License:* MPL-2.0
