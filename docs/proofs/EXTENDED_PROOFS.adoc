...
1. link:#1-complete-substitution-lemma[Complete Substitution Lemma]
2. link:#2-full-progress-proof[Full Progress Proof]
3. link:#3-full-preservation-proof[Full Preservation Proof]
4. link:#4-strong-normalization[Strong Normalization]
5. link:#5-logical-relations-proofs[Logical Relations Proofs]
6. link:#6-resource-safety-complete-proof[Resource Safety Complete Proof]
7. link:#7-shadow-price-convergence[Shadow Price Convergence]
8. link:#8-adequacy-theorem[Adequacy Theorem]
9. link:#9-decidability-proofs[Decidability Proofs]
10. link:#10-complexity-lower-bounds[Complexity Lower Bounds]
...
- `(λx:T₁. γ(e')) v ⟶ γ(e')[x := v] = γlink:e'[x ↦ v]`.
- By IH with `γ[x ↦ v] ∈ 𝓡⟦Γ, x:T₁⟧`: `γlink:e'[x ↦ v] ∈ 𝓡⟦T₂⟧`.
