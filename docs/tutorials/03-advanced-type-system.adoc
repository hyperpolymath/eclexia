...
*Target Audience:* Advanced - understanding of type systems and
programming language theory *Estimated Time:* 4-5 hours *Prerequisites:*
Resource-aware programming, basic type theory
...
[arabic]
. link:#type-system-overview[Type System Overview]
. link:#dimensional-analysis-deep-dive[Dimensional Analysis Deep Dive]
. link:#type-inference-algorithm[Type Inference Algorithm]
. link:#generic-programming[Generic Programming]
. link:#trait-system[Trait System]
. link:#subtyping-and-variance[Subtyping and Variance]
. link:#type-level-computation[Type-Level Computation]
. link:#effect-system[Effect System]
. link:#implementing-type-checked-dsls[Implementing Type-Checked DSLs]
. link:#type-system-internals[Type System Internals]
...
* No null/undefined (Option instead)
...
In a context where x is Int and y is Float, the expression `+x + 1+` has
type Int.

==== Well-Typedness

A program is *well-typed* if all expressions can be assigned types such
that:

[arabic]
. *Type Safety*: Operations are only applied to compatible types
. *Dimensional Correctness*: Physical units are consistent
. *Resource Validity*: All resource annotations are satisfiable
...
Internally, dimensional types are *phantom types* - they exist only at
compile-time:
...
At runtime, `+Quantity<Energy>+` and `+Quantity<Time>+` are both just
`+f64+`. The dimension information is *erased after type-checking*.
...
*Inference algorithm:* 1. Parse literal suffix (J, s, m, kg, etc.) 2.
Look up dimension in dimension table 3. Assign `+Quantity<D>+` type to
literal 4. Propagate dimensions through expressions
...
Eclexia uses a *bidirectional type checking* extension of
Hindley-Milner:

*Three modes:* 1. *Inference mode* (↑): Synthesize type from expression
2. *Checking mode* (↓): Check expression against expected type 3.
*Analysis mode*: Combine both
...
Inference: 1. Assign type variable `+α+` to `+x+` 2. Body has type `+α+`
(just returns x) 3. Function type is `+α → α+` 4. Generalize:
`+∀α. α → α+`
...
Modern type inference generates *constraints* instead of directly
unifying:
...
Eclexia supports *higher-kinded types* for abstraction over type
constructors:
...
Type `+S+` is a *subtype* of type `+T+` (written `+S <: T+`) if a value
of type `+S+` can be used wherever a value of type `+T+` is expected.
...
✅ *Dimensional analysis* - Type-safe physical units ✅ *Type inference*
- Hindley-Milner with bidirectional checking ✅ *Generic programming* -
Polymorphic functions and data structures ✅ *Trait system* - Ad-hoc
polymorphism and abstraction ✅ *Subtyping and variance* - Safe type
hierarchies ✅ *Type-level computation* - Compile-time verification ✅
*Effect system* - Tracking side effects in types ✅ *DSL implementation*
- Building type-safe embedded languages ✅ *Type checker internals* -
How it all works under the hood

==== Next Steps

*Tutorial 4: Economics-as-Code* Apply type system knowledge to economic
modeling: shadow pricing theory, market equilibrium, optimal resource
allocation, and quantitative economics research.

==== Advanced Topics

[arabic]
. *Formal Verification*: Prove type soundness with Coq
. *Refinement Types*: Add logical predicates to types
. *Session Types*: Type-check communication protocols
. *Gradual Typing*: Mix static and dynamic typing
. *Effect Inference*: Infer effects without annotations

'''''

*Total Words:* ~5,100

This tutorial provides a comprehensive deep-dive into Eclexia’s type
system for advanced users and language implementers.
