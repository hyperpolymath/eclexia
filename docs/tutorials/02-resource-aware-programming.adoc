...
*Estimated Time:* 3-4 hours *Prerequisites:* Basic Eclexia syntax,
resources, dimensional types
...
[arabic]
. link:#introduction-to-resource-aware-computing[Introduction to
Resource-Aware Computing]
. link:#multi-resource-management[Multi-Resource Management]
. link:#adaptive-execution[Adaptive Execution]
. link:#carbon-aware-scheduling[Carbon-Aware Scheduling]
. link:#battery-conscious-mobile-apps[Battery-Conscious Mobile Apps]
. link:#cost-optimized-cloud-services[Cost-Optimized Cloud Services]
. link:#real-time-resource-constraints[Real-Time Resource Constraints]
. link:#advanced-patterns[Advanced Patterns]
. link:#performance-and-profiling[Performance and Profiling]
...
Modern computing faces a *resource trilemma*: 1. *Performance* - Users
want fast, responsive applications 2. *Efficiency* - Data centers
consume 1-2% of global electricity 3. *Cost* - Cloud bills scale with
compute usage

Traditional programming treats resources as infinite. Eclexia makes them
...
Problems: - Unknown energy cost (could drain mobile battery) - Unknown
time cost (might timeout) - Unknown $ cost (cloud bill surprise)
...
Benefits: - *Budgets prevent overruns* - If you only have 100J battery
left, ML model won’t run - *Adaptive fallback* - Gracefully degrades to
simpler algorithm - *Predictable costs* - Know exactly what you’re
spending
...
Some resources are *interdependent*. For example, more time usually
costs more energy:
...
If you have plenty of memory but limited time, Eclexia chooses `+fast+`.
If memory is scarce, it chooses `+slow+`.
...
Eclexia automatically: 1. *Evaluates costs* - Checks each
implementation’s shadow price 2. *Checks feasibility* - Ensures budget
isn’t exceeded 3. *Selects optimal* - Chooses lowest-cost feasible
option
...
Selection depends on *current shadow prices*: - *Desktop PC* (unlimited
power): GPU wins (fastest) - *Mobile phone* (battery constrained): CPU
basic wins (lowest energy) - *Embedded device* (limited memory): CPU
basic wins (smallest memory)
...
Tries cache first (cheap), falls back to network (expensive) if cache
misses.
...
Electricity’s carbon footprint varies by: - *Time of day* - Solar power
peaks at noon - *Location* - Renewable vs. coal grids - *Weather* - Wind
turbines produce more on windy days
...
Shadow prices reflect current pricing, automatically routing to cheapest
provider.
...
*Optimize for carbon* (defer work to low-carbon periods) ✅ *Conserve
battery* (adapt mobile app behavior) ✅ *Minimize cloud costs* (select
cheapest instances/regions) ✅ *Meet real-time deadlines* (verify time
constraints) ✅ *Profile and optimize* (identify resource hotspots)

==== Next Steps

*Tutorial 3: Advanced Type System* Dive deep into dimensional analysis,
type inference, generic programming, and implementing your own
type-checked DSLs.

*Tutorial 4: Economics-as-Code* Master shadow pricing theory, implement
market equilibrium models, and use Eclexia for quantitative economics
research.

==== Practice Projects

[arabic]
. *Carbon-Aware CI/CD*: Schedule builds when renewable energy is
abundant
. *Battery Saver App*: Adaptive video player that reduces quality when
battery is low
. *Multi-Cloud Scheduler*: Route tasks to cheapest cloud provider in
real-time
. *Real-Time Controller*: Control system with hard deadlines and
graceful degradation
. *Energy Dashboard*: Monitor and visualize resource consumption across
services
...
This tutorial equips you with practical patterns for building
resource-aware applications. The next tutorial will explore Eclexia’s
advanced type system in depth.
