...
This document provides the complete formal specification of the Eclexia
programming language, including lexical structure, grammar, type system,
and operational semantics. Eclexia is a statically-typed language
implementing the Economics-as-Code paradigm, featuring resource types
with dimensional analysis, adaptive execution blocks, shadow price
computation, and carbon-aware scheduling.
...
. link:#3-grammar[Grammar]
. link:#4-types[Types]
. link:#5-expressions[Expressions]
. link:#6-declarations[Declarations]
. link:#7-patterns[Patterns]
. link:#8-modules[Modules]
. link:#9-type-system[Type System]
. link:#10-operational-semantics[Operational Semantics]
. link:#11-denotational-semantics[Denotational Semantics]
. link:#12-standard-library[Standard Library]
. link:#13-conformance[Conformance]
...
This specification defines: - Lexical and syntactic structure of Eclexia
programs - Static semantics (type system) - Dynamic semantics
(evaluation) - Standard library interfaces
...
|`+Int8+`, `+Int16+`, `+Int32+`, `+Int64+`, `+Int128+` |Fixed-width
signed integers
...
The typing judgment `+Γ ⊢ e : τ+` states that expression `+e+` has type
`+τ+` in context `+Γ+`.

==== 9.2 Type Inference

Eclexia uses bidirectional type inference: - *Synthesis*: `+Γ ⊢ e ⇒ τ+`
(infer type from expression) - *Checking*: `+Γ ⊢ e ⇐ τ+` (check
...
[arabic]
. Accept all syntactically valid programs as defined in §3
. Reject all ill-typed programs as defined in §9
. Implement all typing rules from PROOFS.md
. Implement all reduction rules from §10
. Provide the core standard library from §12.1
. Support resource types and dimensional analysis
...
[arabic]
. Maximum integer size (minimum: 64-bit)
. Floating-point precision beyond IEEE 754
. Maximum recursion depth
. Maximum memory allocation
. Carbon intensity data sources
. Shadow price computation algorithm (must satisfy §8 properties)
...
. Distributed adaptive selection
. IDE integration
. Profile-guided optimization

==== 13.4 Extensions

Implementations MAY provide extensions marked with `+#[extension]+`.
Extensions: - MUST NOT change semantics of conforming programs - MUST be
documented - MUST be opt-in
