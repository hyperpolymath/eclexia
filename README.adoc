...
Eclexia introduces Economics-as-Code: a revolutionary programming paradigm where economic principles—scarcity, trade-offs, and optimization—are built into the language itself.
...
👋 New to Eclexia? Start with the White Paper (Section 1-3) or Tutorial
🔬 Want the theory? Read the Formal Proofs and Language Specification
💼 Business/Legal interests? See IP Claims for patent strategy and commercialization
👨‍💻 Ready to code? Check out Examples and Getting Started
...
let time: Time = 5s
let power: Power = energy / time  // ✓ Correctly typed as Watts

// let invalid = energy + time  // ✗ Compile error: dimension mismatch!
...
The runtime computes shadow prices—the marginal value of each resource—to guide decisions:

def analyze() -> Unit
    @observe: shadow_prices
{
    // Runtime automatically computes:
    // - λ_energy: how valuable is 1 more Joule?
    // - λ_time: how valuable is 1 more millisecond?
    // - λ_carbon: how valuable is 1g less CO2?
...
📊 Expected Benefits (Projected)

NOTE: These are theoretical projections based on the language design. Measured benchmarks are not yet available.
...
Determinism: same context → same selection

🏗️ Project Structure

eclexia/
├── README.md                 # This file
├── WHITEPAPER.md            # Comprehensive introduction
├── PROOFS.md                # Formal mathematical proofs
├── IP_CLAIMS.md             # Intellectual property strategy
...
│   │   ├── shadow/         # Shadow price computation
...
Phase 1: Core Compiler (Q4 2025) — Complete

✅ Language specification (draft)
✅ Core compiler pipeline (Lexer → Parser → AST → TypeCheck → HIR → MIR → Bytecode → VM)
  - ✅ Lexer with 95+ token types including dimensional literals (893 lines)
  - ✅ Pratt parser with full expression and item parsing (3106 lines)
  - ✅ Hindley-Milner type checker with Robinson unification (2210 lines)
  - ✅ HIR lowering (desugaring, type annotations)
  - ✅ MIR lowering (CFG representation, basic optimizations)
  - ✅ Bytecode code generator
  - ✅ Stack-based VM with resource tracking (934 lines)
  - ✅ Tree-walking interpreter (separate execution path, 28 builtin tests)
✅ Standard library (6 modules: core, collections, math, I/O, text, time — 1280 lines .ecl)
✅ Developer toolchain
  - ✅ CLI (build, run, check, fmt, lint, repl, init, new, test, bench, debug, doc, install)
  - ✅ Project minter with 10 templates (bin, lib, web, cli, mcp, ssg, lsp, tool, framework, db-connector)
  - ✅ REPL with expression evaluation
  - ✅ Test runner and benchmark runner
  - ✅ Package manager (manifest parsing, dependency resolution — registry is stub)
  - ✅ LSP server (diagnostics, completion, go-to-def, references, symbols, hover, rename, signature help, formatting)
  - ✅ Formatter (eclexia-fmt: functions, traits, impls, modules, effects)
  - ✅ Linter (configurable rules)
  - ✅ Interactive debugger (breakpoints, step, continue, stack/locals/callstack/resources inspection)
✅ 246 library tests passing, zero warnings, 25 crates compiling
✅ Documentation (specification, getting started guide, whitepaper, theory)

Phase 2: Ecosystem Foundation (Q1–Q2 2026) — In Progress

✅ License migration to PMPL-1.0-or-later
🔄 Core ecosystem libraries (json, http, crypto, regex, log — not yet started)
🔄 TEA web framework (planned)
🔄 Database connectors (Idris2 ABI + Zig FFI pattern — templates ready)
🔄 Package registry (HTTP client exists, server not yet deployed)
🔄 Playground / web REPL (not yet started)
🔄 Eclexia website (not yet started)
✅ odds-and-sods-package-manager integration (Eclexia registry adapter committed)
🔄 Codebase annotation coverage improvement

Phase 3: Advanced Compiler (Q3 2026)

Reactive compilation via Salsa incremental database (structural crates exist, not wired in)
Parallel module compilation
Error recovery for resilient parsing
Compile-time evaluation (comptime)
Abstract interpretation for resource analysis
Algebraic effect compilation (evidence-passing)
Partial evaluation and adaptive specialization

Phase 4: Multi-Target + Production (Q4 2026)

Native backends (Cranelift for dev, LLVM for release — currently stubs that estimate sizes)
WebAssembly backend (currently stub)
Tiered execution (interpreter → VM → Cranelift → LLVM)
Profile-guided optimization
BytecodeModule serialization (file output)
Formal verification completion (some Coq/Agda theorems currently Admitted)
Production deployment infrastructure (Docker/Kubernetes manifests exist, untested)

Known Limitations (Current)

Backends (Cranelift, LLVM, WASM) are structural stubs — they estimate code sizes, not generate real machine code
Runtime modules (scheduler, profiler, carbon, shadow) are 6-line stubs
10 reactive compiler crates exist structurally but are not wired into CLI commands
BytecodeModule does not implement Serialize (no bytecode file output yet)
Package registry server does not exist yet (client is a stub)
Formal verification has Admitted/unproven theorems
No benchmarks with measured results yet (projections only)
...
Sustainable computing (Barroso, Hölzle)
Type theory (Hindley, Milner, Martin-Löf)
...
Patent Grant: If you use Eclexia under the PMPL-1.0-or-later license, you receive a royalty-free patent license for patents embodied in the open-source code.
Trademarks

Eclexia™ is a trademark of Jonathan D.A. Jewell / Eclexia Project

Economics-as-Code™ is a trademark of Jonathan D.A. Jewell / Eclexia Project
