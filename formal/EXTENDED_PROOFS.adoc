...
*Status:* In Progress *Date:* 2026-02-07 *License:* MPL-2.0

'''''

=== Overview

This document catalogs all mechanically-verified proofs for the Eclexia
programming language. Formal verification establishes correctness
properties that are impossible to guarantee through testing alone.

*Proof Assistants Used:* - *Coq* (v8.18+) - Shadow pricing and type
system soundness - *Agda* (v2.6.4+) - Resource tracking and
...
*Lines:* 400+ *Status:* Partially complete (main theorems stated, key
lemmas admitted)

===== Core Definitions

* `+LinearProgram+` - LP in standard form (maximize c^T x subject to Ax
≤ b, x ≥ 0)
...
*Significance:* Establishes that primal objective equals dual objective
at optimum. Foundation for shadow price correctness.
...
*Significance:* Core correctness theorem - proves that dual variables
computed by the simplex algorithm accurately represent marginal values.
...
*Significance:* If a resource constraint isn’t binding, its marginal
value is zero.
...
*Significance:* Positive shadow price implies the constraint is at its
limit.
...
*Significance:* Shadow prices are always non-negative for maximization
problems.
...
*Significance:* Shadow prices increase monotonically as resources become
scarce.
...
*Significance:* Eclexia’s exponential pricing function preserves
monotonicity.
...
*Lines:* 450+ *Status:* Core theorems stated, key lemmas admitted
...
*Significance:* Well-typed programs don’t get stuck. Either they’re
finished (values) or can take a step.
...
*Significance:* Types are preserved during evaluation. If e has type T
and steps to e’, then e’ also has type T.
...
*Significance:* Well-typed programs either evaluate to values or can
continue stepping. No runtime type errors.
...
*Significance:* Dimensional values have correct dimensions at runtime.
...
*Lines:* 280+ *Status:* Main structure complete, some lemmas admitted
...
* `+ConsumesResource+` - Actual resource consumption (instrumented
semantics)
...
*Significance:* Core correctness theorem - tracked usage equals actual
consumption.
...
*Significance:* Resource usage never decreases - consumption is
monotonic.
...
*Significance:* Budgets don’t change during execution - only usage
increases.
...
*Significance:* Budget exhaustion is deterministic - same inputs always
produce same result.
...
*Significance:* Can exactly compute remaining budget and predict
success/failure.
...
*Significance:* Resource consumption composes additively - no hidden
costs.
...
*Significance:* Usage is always non-negative (can’t have negative
consumption).
...
✅ *Non-negativity* - Shadow prices are always ≥ 0 ✅ *Monotonicity* -
Increase with scarcity ⏳ *Strong duality* - Primal = dual at optimum
(stated) ⏳ *Marginal value* - Dual variables are shadow prices (stated)
...
✅ *Progress* - Programs don’t get stuck ✅ *Preservation* - Types
preserved by evaluation ✅ *Soundness* - Combination of progress +
preservation ⏳ *Dimensional safety* - No runtime dimension errors
(stated)
...
✅ *Usage monotonicity* - Never decreases ✅ *Budget constant* - Budgets
don’t change ✅ *Exhaustion determinism* - Predictable failure ✅
*Composition additivity* - Resources compose ✅ *Non-negativity* -
Always ≥ 0 ⏳ *Tracking soundness* - Tracked = actual (stated) ⏳
...
[arabic]
. *Choose an admitted theorem* from above
. *Complete the proof* in Coq or Agda
. *Verify it compiles* without errors
. *Submit a pull request* with proof and explanation

*Required:* - Mechanically verified (no `+admit+` or postulates) -
Documented with comments - Follows existing proof style
...
. *Dantzig, G. B., & Thapa, M. N.* (1997). _Linear Programming 1:
Introduction_. Springer.
. *Bertsekas, D. P.* (1999). _Nonlinear Programming_ (2nd ed.). Athena
Scientific.

==== Papers

[arabic]
. *Hoffmann, J., & Hofmann, M.* (2010). "`Amortized Resource Analysis
with Polymorphic Recursion and Partial Big-Step Operational Semantics.`"
_APLAS 2010_.
...
Solutions to Linear Programs.`" _Bulletin of the American Mathematical
Society_.
...
*Document Version:* 1.0.0 *Last Updated:* 2026-02-07 *Maintainer:*
Jonathan D.A. Jewell *License:* MPL-2.0
