== Changelog

All notable changes to `+eclexia+` will be documented in this file.

This file is generated from conventional commits by the
https://github.com/hyperpolymath/standards/blob/main/.github/workflows/changelog-reusable.yml[`+changelog-reusable.yml+`]
workflow (`+hyperpolymath/standards#206+`). Adopt the workflow in this
repo’s CI to keep this file in sync automatically — see
https://github.com/hyperpolymath/standards/blob/main/templates/cliff.toml[`+templates/cliff.toml+`]
for the canonical config.

The format follows https://keepachangelog.com/en/1.1.0/[Keep a
Changelog]; this project aims to follow
https://semver.org/spec/v2.0.0.html[Semantic Versioning].

=== [Unreleased]

==== Added

* ci(bench): criterion parser benchmark (compiler/eclexia-parser) +
  weekly `bench.yml` — first measured-benchmark harness (`just bench`)
* chore(citation): root CITATION.cff and CONTRIBUTING.adoc (estate docs gate)
* feat: implement S-expression and JSON AST dump for parse command
* feat: add DAP support for eclexia
* feat: add AST visitor framework (eclexia-ast::visitor)
* feat: add parse command for AST visualization (eclexia parse)

==== Changed

* chore(deps): eclexia-lsp migrated tower-lsp 0.20 -> tower-lsp-server
  0.21 (lsp-types 0.97 `Url` -> `Uri`); eclexia-db migrated salsa 0.26 ->
  0.28 (`returns(clone)` keeps owned getter/return semantics)
* docs(proof-debt): record per-axiom keep-decisions with named
  dependencies; decide the weak_duality objective-consistency hypothesis
  (issue #43)

==== Fixed

* fix(deps): salsa 0.28.5 (RUSTSEC-2026-0308), rustls >= 0.23.45
  (RUSTSEC-2026-0285), hashbrown 0.14 removed via cranelift 0.117 +
  tower-lsp-server 0.21/dashmap 6 (RUSTSEC-2024-0402) — issue #80
* fix(ci): re-pin governance/hypatia/scorecard/secret-scanner/mirror
  wrappers to standards@da2c748; workflow-linter SPDX check reads the
  header block; CodeQL held at estate-approved v4.38.0 with Dependabot
  ignore; scorecard/secret-scanner job permissions tightened
* fix(readme): repair broken documentation links (.md -> .adoc),
  canonical repository URL, licence wording
* fix(ci): sync hypatia-scan.yml to canonical (413:
env.HOME+Phase-2+SARIF) (#15)
* fix(ci): adopt canonical hypatia-scan.yml (env.HOME/scanner-layout +
Comment-step gate) (#12)
* fix(ci): move secret-scanner Cargo.toml gate from job-level if: to
step-level (#10)
* fix: ShadowPrices.v compiles clean, MIR macro expansion emits runtime
intrinsic
* fix: repair broken crates and implement concurrency/array stubs
* fix(formal): resolve 3 Idris2 proof holes in Layout.idr
* fix(formal): resolve Agda hole in ResourceTracking.agda
* fix: seam audit — SPDX headers, copyright lines, consistency fixes

==== Documentation

* docs(readme): add SPDX header, OSSF and GWF badges

==== CI

* build(deps): Bump actions/checkout in the actions group (#18)
* build(deps): Bump the actions group with 3 updates (#17)
* build(deps): Bump the actions group across 1 directory with 3 updates
(#13)
* ci: bump actions/upload-artifact SHA to current v4 (#9)
* ci: fix workflow-linter YAML parse error + self-flag bug

=== Pre-history

Prior commits to this file’s introduction are recorded in git history
but not formally classified into Keep-a-Changelog sections. To backfill,
run `+git cliff -o CHANGELOG.md+` locally using the canonical
https://github.com/hyperpolymath/standards/blob/main/templates/cliff.toml[`+cliff.toml+`]
— this is one-shot mechanical work.

'''''
