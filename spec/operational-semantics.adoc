...
`+eclexia-interp/src/{eval,value,env,error,builtins}.rs+`
...
* `+Σ+` — Interpreter state (resource tracking, call depth, method
table)
* `+ρ, Σ ⊢ e ⇓ v+` — Big-step: expression `+e+` evaluates to value `+v+`
* `+ρ, Σ ⊢ s ⇒ ρ', Σ'+` — Statement `+s+` produces updated environment
and state
* `+v+` — Runtime value
* `+⊥+` — Error / divergence
...
Errors propagate upward through the evaluation, unwinding the stack
until caught by a handler (match, try operator) or reaching the top
level.

Control flow signals (Return, Break, Continue) are not true errors —
they are caught by their enclosing construct (function body, loop).
...
[arabic]
. *Lexical scoping:* Blocks create child environments; bindings do not
leak.
. *Call depth bounded:* Recursion limited to 1000 frames.
. *Loop iterations bounded:* While/for limited to 10M iterations.
. *Resource monotonicity:* `+energy_used+` and `+carbon_used+` are
monotonically non-decreasing throughout execution.
. *Shadow price monotonicity:* Shadow prices increase with resource
scarcity (proved in `+formal/coq/src/ShadowPrices.v+`).
. *Adaptive optimality:* The selected solution minimises weighted
resource cost among feasible alternatives.
. *Short-circuit evaluation:* `+&&+` and `+||+` do not evaluate the
right operand when the left operand determines the result.
. *Implicit numeric coercion:* Int is widened to Float in mixed
arithmetic; no implicit narrowing.
