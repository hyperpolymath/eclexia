...
*Version:* 1.0.0 *Date:* 2026-03-14 *Extracted from:*
`+eclexia-typeck/src/{lib,unify,infer,env,error}.rs+`,
`+eclexia-ast/src/{types,dimension}.rs+`
...
A type scheme quantifies over free type variables. Monomorphic types
have ᾱ = ∅.
...
Dimensions form an abelian group under multiplication with ten base
exponents:
...
Addition requires matching dimensions; multiplication produces the
product dimension; division produces the quotient dimension.

'''''

=== 3. Type Inference

Eclexia implements Algorithm W (Damas-Milner) with dimensional
extensions.
...
Arithmetic rules include: - Int ⊕ Int → Int (for +, -, *, /, %) - Float
⊕ Float → Float - Int ⊕ Float → Float (implicit widening) - String +
String → String (concatenation) - Comparisons → Bool
...
Priority order: 1. Inherent impl methods (registered by type name) 2.
Trait impl methods 3. Parent environment
...
Constraints are checked declaratively — not solved. The type system
ensures dimensional consistency of constraint expressions but delegates
runtime enforcement to the adaptive function selector.
...
This allows multiple type errors to be reported in a single pass rather
...
[arabic]
. *Soundness* (Type Safety): If `+Γ ⊢ e : τ+` and `+e ⇓ v+`, then
`+v : τ+`. Currently validated by tests; formal proof in
`+formal/coq/src/Typing.v+` (Progress and Preservation theorems).
. *Principal types:* Algorithm W computes the most general type for
every well-typed expression.
. *Decidability:* Type inference is decidable (polynomial in practice,
theoretical exponential worst case for HM).
. *Dimensional consistency:* Addition requires matching dimensions;
multiplication/division follow the group law.
. *Let-polymorphism:* Definitions bound by `+let+` are generalised;
lambda parameters are not.
. *No implicit narrowing:* Int widened to Float, never Float narrowed to
Int.
